...
VkDescriptorSetInfo* VulkanRenderer::draw_getOrCreateDescriptorSet(PipelineInfo* pipeline_info, LatteDecompilerShader* shader)
{
	...
	for (int i = 0; i < textureCount; ++i)
	{
		...
		if (!textureView)
		{
			...
			{
				info.imageLayout = VK_IMAGE_LAYOUT_ATTACHMENT_FEEDBACK_LOOP_OPTIMAL_EXT;
...
void VulkanRenderer::draw_setRenderPass()
{
	...
	const bool shouldBarrier = GetConfig().vk_accurate_barriers && m_state.activePipelineInfo->accurateBarrierOverride != PipelineInfo::BarrierOverride::NEVER || m_state.activePipelineInfo->accurateBarrierOverride == PipelineInfo::BarrierOverride::ALWAYS;
	...
	if (passReusable)
	{
		...
		if (m_state.hasRenderSelfDependency && shouldBarrier)
		{
